...
async function main() {
    ...
    const demoUser = await client_js_1.prisma.user.upsert({
    ...
    const proUser = await client_js_1.prisma.user.upsert({
    ...
    // 3. Strategy Configs
    await client_js_1.prisma.strategyConfig.upsert({
        where: {
            userId_strategyId: {
                userId: demoUser.id,
    ...
    // 4. Sample Signals
    await client_js_1.prisma.signal.createMany({
        data: [
            {
                id: 'sig-seed-1',
                strategyId: 'london-harsi',
    ...
    await client_js_1.prisma.position.createMany({
        data: [
            {
                id: 'pos-seed-1',
                userId: demoUser.id,
    ...
    await client_js_1.prisma.order.createMany({
        data: [
            {
                id: 'ord-seed-1',
                userId: demoUser.id,
    ...
    // 7. Audit log entries
    await client_js_1.prisma.auditLog.createMany({
        data: [
            {
                userId: demoUser.id,
                action: 'USER_LOGIN',
