...
export function getZonedTime(date: Date = new Date(), timeZone: string = LONDON_TZ) {
...
export function getLondonMinutes(date: Date = new Date()): number {
...
export function getSessionState(
  date: Date = new Date(),
  forceLondonWindow: boolean = false
): SessionInfo {
...
export function extractAsianRange(candles: Candle[]): {
  high: number;
  low: number;
  mid: number;
  rangePips: number;
