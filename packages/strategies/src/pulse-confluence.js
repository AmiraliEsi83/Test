...
exports.PulseConfluenceStrategy = void 0;
const shared_1 = require("@harsi/shared");
const market_data_1 = require("@harsi/market-data");
exports.PulseConfluenceStrategy = {
    id: 'pulse-confluence',
    name: 'Pulse Momentum Confluence',
    badge: 'Institutional Momentum',
    summary: 'Multi-factor momentum confluence engine combining EMA 9/21 trend alignment, RSI location zones, MACD histogram directional expansion, and ATR volatility expansion filters. Executes only when all quantitative factors agree.',
    rules: [
    ...
    evaluate(ctx, config) {
        ...
        const emaFastValues = (0, market_data_1.ema)(closes, params.emaFast);
        const emaSlowValues = (0, market_data_1.ema)(closes, params.emaSlow);
        const rsiValues = (0, market_data_1.rsi)(closes, params.rsiPeriod);
        const macdValues = (0, market_data_1.macd)(closes);
        const atrValues = (0, market_data_1.atr)(candles, 14);
        ...
        if (isEmaBullish) {
            ...
            conditions.push({
                ...
                passed: isEmaBullish,
                detail: `EMA 9: ${(0, shared_1.formatPrice)(ctx.symbol, e9)} > EMA 21: ${(0, shared_1.formatPrice)(ctx.symbol, e21)} (Bullish alignment)`,
            });
            conditions.push({
            ...
            conditions.push({
                ...
                detail: `Current ATR: ${(0, shared_1.formatPrice)(ctx.symbol, currentAtr)} vs baseline: ${(0, shared_1.formatPrice)(ctx.symbol, baselineAtr)} (${atrExpansion.toFixed(2)}x)`,
            });
            conditions.push({
                name: 'Price Structure Filter',
                passed: isTrendBullish,
                detail: `Price (${(0, shared_1.formatPrice)(ctx.symbol, currentPrice)}) supported above dynamic EMA baseline`,
            });
            ...
            conditions.push({
                name: `EMA 9 < EMA 21 (${params.emaFast}/${params.emaSlow})`,
                passed: isEmaBearish,
                detail: `EMA 9: ${(0, shared_1.formatPrice)(ctx.symbol, e9)} < EMA 21: ${(0, shared_1.formatPrice)(ctx.symbol, e21)} (Bearish alignment)`,
            });
            conditions.push({
                name: `RSI Location (${currentRsi.toFixed(1)})`,
                passed: isRsiBearish,
            ...
            conditions.push({
                ...
                detail: `Price (${(0, shared_1.formatPrice)(ctx.symbol, currentPrice)}) capped below dynamic EMA baseline`,
            });
            const passedCount = conditions.filter((c) => c.passed).length;
            score = Math.round(50 - (passedCount / conditions.length) * 45);
            if (passedCount >= 4 && cooldownPassed && config.enabled) {
