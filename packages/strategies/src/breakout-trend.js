...
exports.BreakoutTrendStrategy = void 0;
const shared_1 = require("@harsi/shared");
const market_data_1 = require("@harsi/market-data");
exports.BreakoutTrendStrategy = {
    id: 'breakout-trend',
    name: 'Breakout + Trend Confirmation',
    badge: 'Breakout Engine',
    summary: 'Institutional breakout strategy designed to capture directional momentum following session range compression. Combines 50-period trend filter, prior session High/Low breakout, ATR expansion, and volume confirmation.',
    rules: [
    ...
    evaluate(ctx, config) {
        ...
        const trendEma = (0, market_data_1.ema)(closes, params.trendEmaPeriod);
        ...
        const avgVolume = lookbackSlice.reduce((s, c) => s + (c.volume || 100), 0) / lookbackSlice.length || 100;
        ...
        if (isBullishBreakout || isBullishTrend) {
            conditions.push({
                ...
                detail: `Price (${(0, shared_1.formatPrice)(ctx.symbol, currentPrice)}) above EMA 50 (${(0, shared_1.formatPrice)(ctx.symbol, currentEma50)}) with upward slope`,
            });
            conditions.push({
                name: `Range High Breakout (${params.lookbackBars}-bar High)`,
                passed: isBullishBreakout,
                detail: isBullishBreakout
                    ? `Broke cleanly above ${params.lookbackBars}-bar resistance (${(0, shared_1.formatPrice)(ctx.symbol, swingHigh)})`
                    : `Below resistance barrier (${(0, shared_1.formatPrice)(ctx.symbol, swingHigh)})`,
            });
            conditions.push({
            ...
            conditions.push({
                ...
                detail: `Price (${(0, shared_1.formatPrice)(ctx.symbol, currentPrice)}) below EMA 50 (${(0, shared_1.formatPrice)(ctx.symbol, currentEma50)}) with downward slope`,
            });
            conditions.push({
                name: `Range Low Breakdown (${params.lookbackBars}-bar Low)`,
                passed: isBearishBreakout,
                detail: isBearishBreakout
                    ? `Broke cleanly below ${params.lookbackBars}-bar support (${(0, shared_1.formatPrice)(ctx.symbol, swingLow)})`
                    : `Above support barrier (${(0, shared_1.formatPrice)(ctx.symbol, swingLow)})`,
            });
            conditions.push({
                name: `ATR Volatility Expansion`,
                passed: isAtrConfirmed,
