...
import { Signal, SignalCondition, fromPips, formatPrice } from '@harsi/shared';
import { computeHarsi } from '@harsi/market-data';

export const LondonHarsiStrategy: StrategyPlugin = {
  id: 'london-harsi',
  name: 'London HARSI Mean-Reversion',
  badge: 'Session Algo',
  summary:
    'Institutional London open mean-reversion algorithm. Evaluates the Asian session equilibrium midpoint and fires high-probability alerts during the London T-15 open window when distance reaches threshold bands.',
  rules: [
  ...
  evaluate(ctx: StrategyContext, config: StrategyConfig): StrategyEvaluation {
    ...
    const isBuyZone =
      harsi.value <= params.maxBuyThresholdPips && harsi.value >= params.minBuyThresholdPips;
    const isSellZone =
      harsi.value >= params.minSellThresholdPips && harsi.value <= params.maxSellThresholdPips;
