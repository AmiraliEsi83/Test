...
class BacktestingEngine {
    run(strategy, candles, request) {
        const inst = (0, shared_1.getInstrument)(request.symbol);
        ...
        for (let i = lookbackMin; i < candles.length; i++) {
            ...
            if (activeTrade) {
                ...
                if (isClosed) {
                    const { pnlUsd, pnlPips } = (0, shared_1.calcPnl)(request.symbol, activeTrade.side, activeTrade.entryPrice, exitPrice, activeTrade.lots);
                    balance += pnlUsd;
            ...
            if (!activeTrade) {
                ...
                const session = (0, market_data_1.getSessionState)(candleDate, true); // In backtest mode, session is evaluated
                const asianRange = (0, market_data_1.extractAsianRange)(historySlice);
